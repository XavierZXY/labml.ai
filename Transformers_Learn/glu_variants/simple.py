...
class AutogressiveModel(Module):
    def __init__(
        self,
        src_embed: Module,
        encoder: Encoder,
...
class TinyShakespeareDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        Return a sample
        """
        return self.data[idx : idx + self.seq_len], self.data[
...
class Trainer:
    def __init__(self, configs: Configs):
        ...
        # $$FFN_{ReLU}(x)(x, W_1, W_2, b_1, b_2) = \text{ReLU}_1(x W_1 + b_1) W_2 + b_2$$
            ...
            ffn = FeedForward(
                configs.d_model, configs.d_ff, configs.dropout, nn.ReLU()
            )
        # FFN with ReLU activation
        # $$FFN_{GELU}(x)(x, W_1, W_2, b_1, b_2) = \text{GELU}_1(x W_1 + b_1) W_2 + b_2$$
        elif configs.glu_variant == "GELU":
            ffn = FeedForward(
                configs.d_model, configs.d_ff, configs.dropout, nn.GELU()
            )
        else:
        ...
        mha = MultiHeadAttention(
            configs.n_heads, configs.d_model, configs.dropout
        )
