...
class EmbeddingsWithPositionalEncoding(nn.Module):
    def __init(self, d_model: int, n_vocab: int, max_len: int = 5000):
        super().__init__()
        self.linear = nn.Embedding(n_vocab, d_model)
        self.d_model = d_model
        self.register_buffer(
            "positional_encoding", get_positional_encoding(d_model, max_len)
        )
...
class TransformerLayer(nn.Module):
    def __init__(self, d_model: int, self_attn: MultiHeadAttention, src_attn: MultiHeadAttention=None, feed_forward: FeedForward, dropout_prob: float):
        ...
        self.is_save_ff_input = False # Save input to feed forward layer for visualization
    ...
    def forward(self,
        ...
        self.attn = self.self_attn(z, z, z, mask)
        x = x + self.dropout(self.attn)
        if src is not None:
            z = self.norm_src_attn(x)
            attn_src = self.src_attn(z, src, src, src_mask)
...
class Encoder(nn.Module):
    def __init__(self, layer: TransformerLayer, n_layer: int):
        super().__init__()
